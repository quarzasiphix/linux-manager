...
EOT
    ...
    sudo apt install ufw goaccess npm jq node screen unzip neofetch zip trash-cli nginx curl mariadb-server mariadb-client php-sqlite3 php-gd php-mbstring php-pdo-sqlite php-fpm php-cli php-soap php-zip php-xml php-dom php-curl php-mysqli fail2ban certbot python3-certbot-nginx -y
    npm install -g n8n
    npm install -g pm2
}
